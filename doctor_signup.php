...
$fname = $lname = $address = $profession = $email = $dob = "";
...
$spec_stmt = $database->prepare("SELECT id, sname FROM specialties ORDER BY sname ASC");
$spec_stmt->execute();
    ...
    if($_POST['password'] !== $_POST['confirm_password']){
        ...
        if($check_result->num_rows > 0){
            ...
            $_SESSION["personal"] = array(
                ...
                'address' => $_POST['address'],
                'profession' => $_POST['profession'],
                'specialty' => $_POST['profession'],
                'email' => trim(strtolower($_POST['email'])),
                'password' => $_POST['password'],
                'dob' => $_POST['dob']
            );

            $_SESSION['signup_type'] = 'doctor';
...
<title>MedCheck Doctor Sign Up</title>
            ...
            <select name="profession" required>
                ...
                <?php 
                    foreach($specialties as $spec){
                        ...
                        $selected = ($profession == $spec['id']) ? 'selected' : '';

                        echo "<option value='".$spec['id']."' $selected>".$spec['sname']."</option>";
                    }
                ?>
